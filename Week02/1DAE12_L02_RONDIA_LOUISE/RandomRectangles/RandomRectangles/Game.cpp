...
void DrawRandomRectangle() {
	const float lineWidth{ 5.f };
	const Point2f pos{
		std::max(10.f, rand() / float(RAND_MAX) * g_WindowWidth),
		std::max(10.f, rand() / float(RAND_MAX) * g_WindowHeight)
	};
	const float
		maxWidth{ g_WindowWidth - pos.x - lineWidth - 10.f },
		maxHeight{ g_WindowHeight - pos.y - lineWidth - 10.f };
	const float
